...
fit_poly = readRDS("../data/fpout.RDS")
...
names(fit_poly$scores)[map_chr(fit_poly$scores, typeof)!= "double"]
...
```{r}
# Three different methods to extract col_means from the list.

fit_poly[[7]] # First method

fit_poly[["col_means"]]# Second Method

fit_poly$col_means # Third method

```

# Two ways to extract the third element of a col_means
...
$$
That is, element $n$ is the sum of element $n-1$ and the absolute value of the difference between elements $n-3$ and $n-2$ divided by two. For example, if we let $x_1 = 3$, $x_2 = 1$, and $x_3 = 10$, then $x_4$ is
$$
...
tlist <- vector(mode = "list", length = n)

for (i in seq_len(n)) {
  
rnorm(n = random_obs, mean = mn, sd = sigma) %>%
...
pvec_f <- vector(mode = "numeric", length = n)
...
pvec_m_data <- tibble(p = (pvec_m))

ggplot(pvec_m_data, aes(sample = p)) +
...
ggtitle("P-values vs Uniform Distribution Q-Q Plot")

```

**-**From the Q-Q plot above, the linearity of the points suggests that the p-values exhibit uniform distribution and are normally distributed.
